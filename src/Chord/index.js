import { DefaultCategoricalColor, mergeBase, AbstractChart } from 'vizart-core';
...
class Chord extends AbstractChart {
  constructor(canvasId, _userOptions) {
    ...
    this._s = d => {
      return d[this._options.data.source.accessor];
    };
    this._t = d => {
      return d[this._options.data.target.accessor];
    };
    this._l0 = d => {
      return d[this._options.data.links[0].accessor];
    };
    this._l1 = d => {
      return d[this._options.data.links[1].accessor];
    };
  }
  ...
  render(_data) {
    ...
    this._container
      ...
      .attr(
        'viewBox',
        '0 0 ' + this._options.chart.width + ' ' + this._options.chart.height
      );
    this._svg.attr(
      'transform',
      'translate(' +
    ...
    this._matrix = new Matrix()
      ...
      .filter((item, r, c) => {
        return (
          (this._s(item) === r.name && this._t(item) === c.name) ||
  ...
  update() {
    ...
    let _ribbon = ribbon().radius(innerRadius);
    ...
    let gChanged = this._svg.selectAll('g.group').data(matrix.groups(), d => {
      return d._id;
    });
    ...
    let chords = this._svg.selectAll('path.chord').data(matrix.chords(), d => {
      return d._id;
    });
