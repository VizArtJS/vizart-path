...
let VerticalSankey = function() {
  ...
  sankey.link = function() {
    ...
    function link(d) {
      ...
      return (
        ...
        x0 +
        ',' +
        y0 + // start (of SVG path)
        'C' +
        x0 +
        ',' +
        y2 + // CP1 (curve control point)
        ' ' +
        x1 +
        ',' +
        y3 + // CP2
  ...
  function computeNodeBreadths(iterations) {
    ...
    let ky =
      (size[0] - (nodesByBreadth[0].length - 1) * nodePadding) /
      sum(nodesByBreadth[0], value);
