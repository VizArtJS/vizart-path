...
class Sankey extends AbstractChart {
  ...
  update(_data) {
    ...
    links
      ...
      .delay((d, i) => {
        return (
          i / this._data.links.length * this._options.animation.duration.update
        );
      })
      .attr('d', path)
        ...
        if (that._options.plots.horizontal) {
          select(this).attr(
            ...
            'translate(' +
              ...
              (d.y = Math.max(
                0,
                Math.min(that._options.chart.innerHeight - d.dy, event.y)
              )) +
              ')'
          );
        } else {
          select(this).attr(
            ...
            'translate(' +
              (d.x = Math.max(
                0,
                Math.min(that._options.chart.innerWidth - d.dy, event.x)
              )) +
              ',' +
              d.y +
              ')'
    ...
    nodes
      ...
      .delay((d, i) => {
        return (
          i / this._data.nodes.length * this._options.animation.duration.update
        );
      })
      .attr(
        'height',
        this._options.plots.horizontal
          ? d => {
    ...
    if (this._options.plots.horizontal === true) {
      nodes
        ...
        .attr(
          'x',
          this._options.plots.horizontal ? 6 + this._layout.nodeWidth() : 0
        )
    ...
    if (this._options.plots.realTime === false) {
      ...
      let frequencyScale = scaleLinear()
        .domain(linkExtent)
        .range([1, 100]);
      let particleSize = scaleLinear()
        .domain(linkExtent)
        .range([1, 5]);
      ...
      this._data.links.forEach(function(link) {
        ...
        link.particleColor = scaleLinear()
          .domain([1, 1000])
      ...
      function tick(elapsed, time) {
        ...
        selectAll('.sankey-link').each(function(d) {
          if (d.freq >= freqCounter) {
            let offset = (Math.random() - 0.5) * d.dy;
            particles.push({
              link: d,
              time: elapsed,
              offset: offset,
      ...
      function particleEdgeCanvasPath(elapsed) {
        ...
        for (let x in particles) {
          ...
          let currentPercent =
            currentTime /
            options.plots.realTimeSpeed *
            particles[x].path.getTotalLength();
  ...
  _mouseOnNode(d, i) {
    ...
    this._linkGroup
      ...
      .style('stroke', function(_d) {
        return options.plots.colorfulLink
          ? colorScale(_d.source.name)
  ...
  mouseOnPath(d, i) {
    ...
    let html = Tooltip.noHandle(
      options.label.sourceLabel + ' ' + d.source.name,
  ...
  transitionColor(colorOptions) {
    ...
    this._nodeGroup
      ...
      .delay((d, i) => {
        return (
          i / this._data.nodes.length * this._options.animation.duration.color
        );
      })
      .style('fill', d => {
    ...
    this._linkGroup
      ...
      .delay((d, i) => {
        return (
          i / this._data.links.length * this._options.animation.duration.color
        );
      })
      .style('stroke', d => {
        return this._options.plots.colorfulLink
