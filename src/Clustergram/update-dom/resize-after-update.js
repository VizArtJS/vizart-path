...
const resize_after_update = function(params, row_nodes, col_nodes, links, duration, delays) {
    ...
    // resize the svg
    ///////////////////////
    let svg_group = d3.select('#' + params.viz.svg_div_id)
        .select('svg');
    ...
    svg_group.selectAll('.highlighting_rect')
        .transition().delay(delays.update).duration(duration)
        .attr('width', params.matrix.x_scale.bandwidth() * 0.80)
        .attr('height', params.matrix.y_scale.bandwidth() * 0.80);
    ...
    svg_group.selectAll('.tile_split_up')
        ...
        .attr('d', function () {
            ...
            let start_y = 0;
            let final_y = params.matrix.y_scale.bandwidth() - params.matrix.y_scale.bandwidth() / 60;
            let output_string = 'M' + start_x + ',' + start_y + ', L' +
    ...
    svg_group.selectAll('.tile_split_dn')
        ...
        .attr('d', function () {
            let start_x = 0;
            let final_x = params.matrix.x_scale.bandwidth();
            let start_y = params.matrix.y_scale.bandwidth() - params.matrix.y_scale.bandwidth() / 60;
            let final_y = params.matrix.y_scale.bandwidth() - params.matrix.y_scale.bandwidth() / 60;
            let output_string = 'M' + start_x + ', ' + start_y + ' ,   L' +
    ...
    svg_group.selectAll('.row_label_text')
        .each(function () {
            let bbox = d3.select(this).select('text')[0][0].getBBox();
            d3.select(this)
                .select('rect')
                .attr('x', bbox.x)
                .attr('y', 0)
    ...
    d3.selectAll('.row_label_text').each(function () {
        let tmp_width = d3.select(this).select('text').node().getBBox().width;
    ...
    svg_group.selectAll('.row_viz_group')
        ...
        .attr('d', function () {
            ...
            let output_string = 'M ' + origin_x + ',' + origin_y + ' L ' +
    ...
    if (Utils.has(params.network_data.row_nodes[0], 'value')) {
        ...
        // set bar scale
        let enr_max = Math.abs(max(params.network_data.row_nodes, function (d) {
    ...
    let x_offset_click = params.matrix.x_scale.bandwidth() / 2 + params.viz.border_width;
    ...
    svg_group.selectAll('.col_label_click')
        .transition().delay(delays.update).duration(duration)
        .attr('transform', 'translate(' + params.matrix.x_scale.bandwidth() / 2 + ',' + x_offset_click + ') rotate(45)');
    ...
    svg_group.selectAll('.col_label_click')
        .select('text')
        .transition().delay(delays.update).duration(duration)
        .attr('y', params.matrix.x_scale.bandwidth() * 0.60)
    ...
    svg_group.selectAll('.col_label_click')
        ...
        .attr('d', function () {
            ...
            let start_y = -(params.matrix.x_scale.bandwidth() - reduce_rect_width +
            params.viz.border_width);
            let final_y = -params.viz.border_width;
            let output_string = 'M ' + origin_y + ',0 L ' + start_y + ',' +
    ...
    svg_group.select('#super_col')
        ...
        .attr('transform', function () {
            let inst_x = params.viz.clust.dim.width / 2 + params.norm_label.width
                    .row;
    ...
    svg_group.select('#super_row')
        ...
        .attr('transform', function () {
            ...
            let inst_y = params.viz.clust.dim.height / 2 + params.norm_label.width
                    .col;
            return 'translate(' + inst_x + ',' + inst_y + ')';
        });
    ...
    svg_group.select('#right_spillover')
        ...
        .attr('transform', function () {
            ...
            let tmp_top = params.norm_label.margin.top + params.norm_label.width
                    .col;
    ...
    svg_group.select('#bottom_spillover')
        ...
        .attr('transform', function () {
            ...
            let inst_offset = params.viz.svg_dim.height - 3 * params.viz.grey_border_width;
