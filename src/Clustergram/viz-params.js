...
import reset_visualization_size from './update-dom/reset-visualization-size';

import map from 'lodash-es/map';
import max from 'lodash-es/max';
import each from 'lodash-es/each';

const VizParams = function(config) {
  let params = initialize_visualization(config);
  ...
  function initialize_visualization(config) {
    ...
    params.network_data.row_nodes_names = map(row_nodes, 'name');
    params.network_data.col_nodes_names = map(col_nodes, 'name');

    // find the label with the most characters and use it to adjust the row and col margins
    let row_max_char = max(row_nodes, function(inst) {
      return inst.name.length;
    }).name.length;
    let col_max_char = max(col_nodes, function(inst) {
      return inst.name.length;
    }).name.length;
    ...
    let min_num_char = 5;
    let max_num_char = params.labels.max_label_char;

    // number of characters to show
    params.labels.show_char = 15;

    // calc how much of the label to keep
    let keep_label_scale = scaleLinear()
      .domain([params.labels.show_char, max_num_char])
      .range([1, params.labels.show_char / max_num_char])
      .clamp('true');
    ...
    // define label scale
    ///////////////////////////
    let min_label_width = 85;
    let max_label_width = 140;
    let label_scale = scaleLinear()
    ...
    let screen_label_scale = scaleLinear()
      .domain([500, 1000])
      .range([1.0, 1.0])
      .clamp(true);
    ...
    params.norm_label.margin.left =
      params.viz.grey_border_width + params.labels.super_label_width;
    ...
    params.viz.clust.margin.left =
      params.norm_label.margin.left + params.norm_label.background.row;
    params.viz.clust.margin.top =
      params.norm_label.margin.top + params.norm_label.background.col;
    ...
    params.viz.spillover_x_offset =
      label_scale(col_max_char) * 0.7 * params.col_label_scale;
    ...
    params.matrix.orders = {
      // ini
      ini_row: range(params.viz.num_col_nodes).sort(function(a, b) {
        return col_nodes[b].ini - col_nodes[a].ini;
      }),
      ini_col: range(params.viz.num_row_nodes).sort(function(a, b) {
        return row_nodes[b].ini - row_nodes[a].ini;
      }),
      // rank
      rank_row: range(params.viz.num_col_nodes).sort(function(a, b) {
        return col_nodes[b].rank - col_nodes[a].rank;
      }),
      rank_col: range(params.viz.num_row_nodes).sort(function(a, b) {
        return row_nodes[b].rank - row_nodes[a].rank;
      }),
      // clustered
      clust_row: range(params.viz.num_col_nodes).sort(function(a, b) {
        return col_nodes[b].clust - col_nodes[a].clust;
      }),
      clust_col: range(params.viz.num_row_nodes).sort(function(a, b) {
        return row_nodes[b].clust - row_nodes[a].clust;
      }),
      // class
      class_row: range(params.viz.num_col_nodes).sort(function(a, b) {
        return col_nodes[b].cl - col_nodes[a].cl;
      }),
      class_col: range(params.viz.num_row_nodes).sort(function(a, b) {
    ...
    // let min_col_shrink_scale = scaleLinear().domain([100,1500]).range([1,0.1]).clamp('true');
    // let min_col_shrink = min_col_shrink_scale(params.viz.svg_dim.width);
    ...
    params.matrix.x_scale = scaleBand().range([0, params.viz.clust.dim.width]);
    params.matrix.y_scale = scaleBand().range([0, params.viz.clust.dim.height]);
    ...
    params.viz.border_width =
      params.matrix.x_scale.bandwidth() / params.viz.border_fraction;
    ...
    params.matrix.rect_width =
      params.matrix.x_scale.bandwidth() - params.viz.border_width;
    params.matrix.rect_height =
    ...
    let min_node_num = 10;
    let max_node_num = 3000;

    // min and max expected screen widths
    let min_viz_width = 400;
    let max_viz_width = 2000;
    ...
    params.labels.default_fs_col = params.matrix.x_scale.bandwidth() * 0.87;
    ...
    // set opacity scale
    params.matrix.max_link = max(network_data.links, function(d) {
    ...
    if (config.input_domain === 0) {
      ...
      if (params.matrix.opacity_function === 'linear') {
        params.matrix.opacity_scale = scaleLinear()
          .domain([0, Math.abs(params.matrix.max_link)])
          .clamp(true)
          .range([0.0, 1.0]);
      } else if (params.matrix.opacity_function === 'log') {
        params.matrix.opacity_scale = d3.scale
          .log()
          .domain([0.001, Math.abs(params.matrix.max_link)])
          .clamp(true)
          .range([0.0, 1.0]);
      }
    } else {
      ...
      if (params.matrix.opacity_function === 'linear') {
        params.matrix.opacity_scale = scaleLinear()
          .domain([0, config.input_domain])
          .clamp(true)
          .range([0.0, 1.0]);
      } else if (params.matrix.opacity_function === 'log') {
        params.matrix.opacity_scale = d3.scale
          .log()
          .domain([0.001, config.input_domain])
          .clamp(true)
          .range([0.0, 1.0]);
      }
    }
    ...
    if (
      ...
      Utils.has(network_data.links[0], 'highlight')
    ) {
  ...
  function initialize_resizing(params) {
    ...
    if (params.viz.expand_button) {
      ...
      if (d3.select('#expand_button').empty()) {
        ...
        exp_button = d3.select('#expand_button');
      }
  ...
  function initialize_matrix(network_data) {
    ...
    each(network_data.row_nodes, function(tmp, row_index) {
      matrix[row_index] = range(network_data.col_nodes.length).map(function(
        col_index
      ) {
        return {
          pos_x: col_index,
    ...
    each(network_data.links, function(link) {
