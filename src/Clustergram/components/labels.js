...
const Labels = function(params) {
    ...
    function make_rows(params, row_nodes, reorder, text_delay) {
        ...
        let row_labels = d3.select('#row_label_zoom_container')
            .selectAll('g')
        ...
        let row_viz_group = d3.select('#row_viz_zoom_container')
            .selectAll('g')
            .data(row_nodes, function (d) {
                return d.name;
            })
        ...
        if (Utils.has(params.network_data.row_nodes[0], 'value')) {
            ...
            // set bar scale
            let enr_max = Math.abs(max(row_nodes, function (d) {
    ...
    function make_cols(params, col_nodes, reorder, text_delay) {
        ...
        if (d3.select('#col_container').empty()) {
            ...
            container_all_col = d3.select('#main_svg')
                .append('g')
            ...
            let container_all_col = d3.select('#col_container')
                .attr('transform', 'translate(' + params.viz.clust.margin.left + ',' +
                    params.norm_label.margin.top + ')');
        ...
        let col_label_obj = d3.select('#col_label_zoom_container')
        ...
        // get max value
        let enr_max = Math.abs(max(col_nodes, function (d) {
            return Math.abs(d.value)
        }).value);
        let enr_min = Math.abs(min(col_nodes, function (d) {
            return Math.abs(d.value)
        }).value);
            ...
            if (id_clicked_col != params.click_hlight_col) {
                ...
                d3.select(clicked_col)
                    ...
                    .attr('transform', function () {
                        ...
                        let tmp_translate_y = params.matrix.x_scale.bandwidth() - hlight_width;
                        let tmp_translate_x = -(params.viz.clust.dim.height +
                        params.class_room.col + params.viz.uni_margin);
                        return 'translate(' + tmp_translate_x + ',' + tmp_translate_y + ')';
                    });
