...
const Zoom = function(params) {
    ...
    function apply_transformation(params, trans_x, trans_y, zoom_x, zoom_y) {
        ...
        // scale is greater than params.viz.zoom_switch
            ...
            // multiple extra room (zoom - 1) by the width
            let pan_room_x = (d3_scale / params.viz.zoom_switch - 1) * params.viz.clust.dim.width;
    ...
    function two_translate_zoom(params, pan_dx, pan_dy, fin_zoom) {
        ...
        if (!params.viz.run_trans) {
            ...
            if (pan_dy >= half_height - y_pan_room) {
                ...
                let shift_up_viz = -half_height / params.viz.zoom_switch +
                    shift_top_viz;
    ...
    function update_viz_links(params, trans_x, trans_y, zoom_x, zoom_y, trans) {
        ...
        let buffer = 1;
        let min_x = Math.abs(trans_x) / zoom_x - buffer * params.matrix.x_scale.bandwidth();
        let min_y = Math.abs(trans_y) / zoom_y - buffer * params.matrix.y_scale.bandwidth();

        let max_x = Math.abs(trans_x) / zoom_x + params.viz.clust.dim.width / zoom_x;
        // let max_y = Math.abs(trans_y)/zoom_y + params.viz.clust.dim.height ;
        let max_y = Math.abs(trans_y) / zoom_y + params.viz.clust.dim.height / zoom_y;
    ...
    function constrain_font_size(params, trans) {
            ...
            d3.selectAll('.row_label_text').each(function () {
                if (trans) {
                    d3.select(this).select('text')
                        ...
                        .attr('y', params.matrix.y_scale.bandwidth() * params.scale_font_offset(params.viz.zoom_scale_font.row));
                } else {
                    d3.select(this).select('text')
                        .style('font-size', params.labels.default_fs_row * params.viz.zoom_scale_font.row + 'px')
                        .attr('y', params.matrix.y_scale.bandwidth() * params.scale_font_offset(params.viz.zoom_scale_font.row))
        ...
        function trim_text(inst_selection, inst_rc) {
            ...
            let tmp_width = d3.select(inst_selection).select('text').node().getBBox().width;
            let inst_text = d3.select(inst_selection).select('text').text();
    ...
    function downsample(params, min_rect_height) {
        ...
        let color_scale = scaleLinear().domain([-1, 1])
