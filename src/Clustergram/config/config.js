...
const Config = function(args) {
  ...
  if (config.transpose) {
    ...
    let tmp_col_label = args.col_label;
    let tmp_row_label = args.row_label;
  ...
  config.show_dendrogram =
    Utils.has(args.network_data.row_nodes[0], 'group') ||
    Utils.has(args.network_data.col_nodes[0], 'group');
  config.show_categories =
    Utils.has(args.network_data.row_nodes[0], 'cl') ||
    Utils.has(args.network_data.col_nodes[0], 'cl');
  ...
  if (config.show_categories) {
    ...
    let class_rows = uniq(map(args.network_data.row_nodes, 'cl'));
    config.class_colors.row = {};
    each(class_rows, function(c_row, i) {
      if (i === 0) {
    ...
    // associate classes with colors
    let class_cols = uniq(map(args.network_data.col_nodes, 'cl'));
    config.class_colors.col = {};
    each(class_cols, function(c_col, i) {
      if (i === 0) {
