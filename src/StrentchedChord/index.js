import { AbstractChart, wrapSVGText, DefaultCategoricalColor, mergeBase } from 'vizart-core';
...
class StretchedChord extends AbstractChart {
    ...
    render(_data) {
        ...
        let outerRadius = Math.min(this._options.chart.innerWidth, this._options.chart.innerHeight) / 2  - (this._isMobileSize ? 80 : 100);
        ...
        //////////////////// Titles on top ///////////////////
        //////////////////////////////////////////////////////
        let wrapper = this._svg.append("g").attr("class", "chordWrapper")
            .attr("transform", "translate(" + this._options.chart.width / 2 + "," + this._options.chart.height / 2 + ")");
        ...
        titleWrapper.append("text")
            ...
            .attr("x", (width/2 + this._options.chart.margin.left - outerRadius - titleSeparate))
            .attr("y", titleOffset)
            .text(this._options.data.source.name);
        titleWrapper.append("line")
            .attr("class","titleLine left")
            .attr("x1", (width/2 + this._options.chart.margin.left - outerRadius - titleSeparate)*0.6)
            .attr("x2", (width/2 + this._options.chart.margin.left - outerRadius - titleSeparate)*1.4)
        ...
        titleWrapper.append("line")
            ...
            .attr("x1", (width/2 + this._options.chart.margin.left - outerRadius - titleSeparate)*0.6 + 2*(outerRadius + titleSeparate))
            .attr("x2", (width/2 + this._options.chart.margin.left - outerRadius - titleSeparate)*1.4 + 2*(outerRadius + titleSeparate))
            .attr("y1", titleOffset+8)
            .attr("y2", titleOffset+8);
        ...
        let startAngle =(d)=> { return d.startAngle + offset; }
        let endAngle =(d)=> { return d.endAngle + offset; }

        // Returns an event handler for fading a given chord group
        let fade = (opacity)=> {
            return (d, i)=> {
                wrapper.selectAll("path.chord")
                    .filter((d)=> { return d.source.index !== i && d.target.index !== i && Names[d.source.index] !== ""; })
        ...
        g.append("path")
            .style("stroke", (d,i)=> { return (Names[i] === "" ? "none" : "#00A1DE"); })
            .style("fill",(d,i)=> { return (Names[i] === "" ? "none" : "#00A1DE"); })
            .style("pointer-events", (d,i)=> { return (Names[i] === "" ? "none" : "auto"); })
            .attr("d", _arc)
        ...
        wrapper.selectAll("path.chord")
            ...
            .style("opacity", (d)=> { return (Names[d.source.index] === "" ? 0 : opacityDefault); }) //Make the dummy strokes have a zero opacity (invisible)
            .style("pointer-events", (d,i)=> { return (Names[d.source.index] === "" ? "none" : "auto"); }) //Remove pointer events from dummy strokes
            .attr("d", _chordPath)
