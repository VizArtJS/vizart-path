...
class BiPartite extends AbstractChart {
    ...
    this._data.forEach((biP, s) => {
      ...
      [0, 1].forEach(p => {
        this._svg
          ...
          .selectAll('.mainbar')
          .on('mouseover', (d, i) => {
            return this._selectSegment(p, i);
          })
          .on('mouseout', (d, i) => {
            return this._deSelectSegment(p, i);
          });
      });
    });
  }
  ...
  transitionColor(colorOptions) {
    ...
    let _transition = transition().duration(
      this._options.animation.duration.change
    );

    _transition.selectAll('.subbar').style('fill', d => {
      return this._color(d.key1);
    });

    _transition.selectAll('.edge').style('fill', d => {
      return this._color(d.key1);
    });
  }
  ...
  _drawPart(data, id, p) {
    this._svg
      ...
      .attr(
        'transform',
        'translate(' + (p * (this.bb + b) + this._options.plots.gap) + ',0)'
      );
    this._svg
      .select('#' + id)
      .select('.part' + p)
