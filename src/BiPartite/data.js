...
const _buildPartite = (data, _p)=>{
    ...
    sData.keys=[
        set(data.map((d)=>{ return d[0];})).values().sort((a,b)=>{ return ( a<b? -1 : a>b ? 1 : 0);}),
        set(data.map((d)=>{ return d[1];})).values().sort((a,b)=>{ return ( a<b? -1 : a>b ? 1 : 0);})
    ];

    sData.data = [
        sData.keys[0].map( (d)=> { return sData.keys[1].map( (v)=> { return 0; }); }),
        sData.keys[1].map( (d)=> { return sData.keys[0].map( (v)=>{ return 0; }); })
    ];
    ...
    return [
        {
            ...
            id:'part_01',
            header:[_opt.data.source.name, _opt.data.target.name, _opt.data.links[0].name]
        },
        {
            data: _buildPartite(temp, 3),
            id:'part_02',
            header:[_opt.data.source.name, _opt.data.target.name, _opt.data.links[1].name]
        }
    ]
};
